...
```
index.html                 主頁面（完整版）
office.html                辦公室版：頁首＋三個頁籤，萊姆綠系，適合 iframe 嵌入
...
|---|---|---|
| 色系 | 玫瑰粉 `#A83459` | 萊姆綠 `#B4D62B` |
| 標誌 | 白粉漸層愛心 48×44 | 淺萊姆星星 58×55 |
| 標題列文字 | 白 | 深橄欖綠 `#24310A` |

亮萊姆綠底配白字只有 **1.7:1**，完全看不清楚，所以 office 的標題列一律改用深色文字
（對漸層三個色階分別為 9.52 / 8.29 / 6.03，皆達 WCAG AA）；
副標為半透明，混色後在最深端只有 4.44:1，因此透明度提高到 .88 才全段 ≥ 4.8:1。
